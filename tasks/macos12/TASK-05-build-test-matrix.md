...
- [x] 在 CI 执行 `make check`、完整分片 Swift tests 与 provider engine goldens，全部通过。
- [x] 产出一条可复制的 release 构建命令。
- [x] 将构建产物传回目标 Intel macOS 12.7.6 主机并完成 `MAC12-06` 实测。
...
- Artifact SHA256：6f0bdfecdd6b16226b462163529492bd64a46fd20199067a4a27447079a0c71c（最终 CI 产物）。
...
- 最终绿色 CI：https://github.com/reycerwillys-dev/CodexBar/actions/runs/31555175138
- make check、两个 Swift test shard、provider plugin engine goldens、Intel test-target compile 与 release package 全部通过。
- 9 个 Mach-O 验证通过；主 App、CLI、watchdog 和 Widget 为 x86_64/minOS 12.0，Sparkle 为 universal/minOS 10.13。
- Artifact SHA-256：6f0bdfecdd6b16226b462163529492bd64a46fd20199067a4a27447079a0c71c。
